...
- 提交信息需清晰描述本次改动
- 除非遇到鉴权失败、网络失败、敏感信息风险或 Leo 明确禁止，否则不跳过 push

## Rule 25 — 涉及敏感信息时禁止推送（2026-04-09 新增）

如果一次推送可能包含我或 Leo 的密钥、密码、Token、API Key、私密凭证或其他敏感信息，禁止执行推送。

**强制要求**：
- 发现敏感信息风险时，立即停止 push，不以“先推再删”作为方案
- 先评估是否真的需要推送；若无必要，则保持本地提交即可
- 若确需远程同步，必须先完成脱敏、移除历史敏感信息，确认干净后再推送
- 安全优先级高于同步优先级，宁可暂不推送，也不能泄露凭证
